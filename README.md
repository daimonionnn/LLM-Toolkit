...
- [ ] **Try AMD's prebuilt `rocm-sdk-libraries-gfx900` instead of the Tensile backport.** AMD publishes a complete ROCm 7.14 library set built for gfx900 as a pip wheel — [`rocm_sdk_libraries_gfx900-7.14.0a20260612`](https://rocm.nightlies.amd.com/v2/gfx900/), 804 MB, containing rocBLAS, rocRAND, rocFFT, rocSPARSE, MIOpen and hipBLAS. That is every library the 2026-09-10 package survey found missing from classic ROCm 7.2, already built, and with `HSA_OVERRIDE_GFX_VERSION=9.0.0` it should serve this iGPU. If it works it replaces this repo's central backport with a no-build install and unblocks the ML stack on ROCm 7 rather than 6.3. Caveats to check: the whole nightly index froze on 2026-06-12 (all architectures, gfx1151 included, so gfx900 was not singled out), and TheRock excludes hipBLASLt for gfx900 even though the `.so` ships
...
- [ ] **Build the ML stack (PyTorch / ComfyUI) on ROCm 6.3.4, not on 7.x.** Package survey 2026-09-10: rocBLAS, rocRAND, rocFFT and rocSPARSE **all ship gfx900 device code in 6.3.4 and none of them do in 7.2** — MIOpen JIT-compiles and does not care. So the backporting this repo is built around does not arise for ML work; it exists only because llama.cpp is wanted on ROCm 7. Do not try to carry the 6.3.4 rocRAND into a 7.x install: it is one `.so` and does contain gfx900, but it links `libamdhip64.so.6`, so it needs a ROCm 6 runtime anyway. Raw data in [bench/results/2026-09-10-rocm-gfx900-library-survey.tsv](bench/results/2026-09-10-rocm-gfx900-library-survey.tsv); this checks what is *present in the package*, not that it runs — the next step is a ROCm 6.3.4 container with a real PyTorch tensor op on the Vega 8. Prompted by [issue #1](https://github.com/daimonionnn/amd-vega-rocm-vulkan-llm-toolkit/issues/1)

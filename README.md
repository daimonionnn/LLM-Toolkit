...
- [ ] **Establish whether rocRAND can be satisfied on gfx900 — the gate on any PyTorch/ComfyUI work here.** [Issue #1](https://github.com/daimonionnn/amd-vega-rocm-vulkan-llm-toolkit/issues/1) reports that MIOpen and rocFFT need **no backport at all** on gfx900 (both JIT-compile through comgr), which removes the blocker these docs assumed applied to the whole ML stack. rocRAND and RCCL are the wall instead: under modular ROCm their kernels ship inside a proprietary `.kpack` container (custom header + zstd), so the copy-from-6.3.4 trick does not reach them; rocSPARSE rejects gfx900 in compiled code regardless. PyTorch needs rocRAND for GPU-side RNG, so answer that one before investing in the rest. Cheapest test: install ROCm 6.3.x classic (which predates `.kpack`) and see whether a stock rocRAND works under the override
